...
import {
  ...
  H,
  Q,
} from '../components/Cropper/Cropper.constants';
...
class CropperPage extends Component {
  constructor(props) {
    ...
    const BOTTOM_VALUE =
      props.BOTTOM_VALUE !== 0 ? props.BOTTOM_VALUE : BOTTOM_LIMIT;
    const RIGHT_VALUE =
      props.RIGHT_VALUE !== 0 ? props.RIGHT_VALUE : RIGHT_LIMIT;

    const topOuterPosition = new Animated.ValueXY({
      x: LEFT_VALUE - BORDER_WIDTH,
      y: TOP_VALUE - BORDER_WIDTH,
    });
    const topOuterPanResponder = new PanResponder.create({
      onStartShouldSetPanResponder: () => false,
    });
    const leftOuterPosition = new Animated.ValueXY({
      x: LEFT_VALUE - BORDER_WIDTH,
      y: TOP_VALUE - BORDER_WIDTH,
    });
    const leftOuterPanResponder = new PanResponder.create({
      onStartShouldSetPanResponder: () => false,
    });
    ...
    const bottomOuterPanResponder = new PanResponder.create({
      onStartShouldSetPanResponder: () => false,
    });
    const rightOuterPosition = new Animated.ValueXY({
      x: SCREEN_WIDTH - RIGHT_VALUE,
      y: TOP_VALUE - BORDER_WIDTH,
    });
    const rightOuterPanResponder = new PanResponder.create({
      onStartShouldSetPanResponder: () => false,
    });

    const topPosition = new Animated.ValueXY({
      x: LEFT_VALUE - BORDER_WIDTH,
      y: TOP_VALUE - BORDER_WIDTH,
    });
    const topPanResponder = this.initSidePanResponder('topPosition');
    const leftPosition = new Animated.ValueXY({
      x: LEFT_VALUE - BORDER_WIDTH,
      y: TOP_VALUE - BORDER_WIDTH,
    });
    const leftPanResponder = this.initSidePanResponder('leftPosition');
    const bottomPosition = new Animated.ValueXY({
      x: LEFT_VALUE - BORDER_WIDTH,
      y: SCREEN_HEIGHT - BOTTOM_VALUE,
    });
    ...
    const topLeftPosition = new Animated.ValueXY({
      x: LEFT_VALUE - BORDER_WIDTH,
      y: TOP_VALUE - BORDER_WIDTH,
    });
    const topLeftPanResponder = this.initCornerPanResponder(
      'topPosition',
      'leftPosition',
    );
    ...
    const bottomLeftPanResponder = this.initCornerPanResponder(
      'bottomPosition',
      'leftPosition',
    );
    ...
    const bottomRightPanResponder = this.initCornerPanResponder(
      'bottomPosition',
      'rightPosition',
    );
    const topRightPosition = new Animated.ValueXY({
      x: SCREEN_WIDTH - RIGHT_VALUE,
      y: TOP_VALUE - BORDER_WIDTH,
    });
    const topRightPanResponder = this.initCornerPanResponder(
      'topPosition',
      'rightPosition',
    );

    const rectanglePosition = new Animated.ValueXY({
      x: LEFT_VALUE,
      y: TOP_VALUE,
    });
  ...
  getTopOuterStyle = () => {
    return {
      ...
      height: Animated.add(
        this.props.BORDER_WIDTH - this.state.TOP_LIMIT,
        this.state.topPosition.y,
      ),
      width: W,
  ...
  getLeftOuterStyle = () => {
    return {
      ...
      width: Animated.add(
        this.props.BORDER_WIDTH - this.state.LEFT_LIMIT,
        this.state.leftPosition.x,
      ),
      backgroundColor: `rgba(0, 0, 0, ${this.props.NOT_SELECTED_AREA_OPACITY})`,
    };
  };
  ...
  getImageStyle = () => {
    const DIFF =
      this.state.topPosition.y._value - this.state.rightPosition.y._value;
    return {
      position: 'absolute',
  ...
  isAllowedToMoveTopSide = gesture => {
    return (
      this.state.topPosition.y._offset + gesture.dy >=
        this.state.TOP_LIMIT - this.props.BORDER_WIDTH &&
      this.state.topPosition.y._offset +
        gesture.dy +
        this.props.BORDER_WIDTH +
        1 <
        this.state.bottomPosition.y._offset
    );
  };
  isAllowedToMoveLeftSide = gesture => {
    return (
      this.state.leftPosition.x._offset + gesture.dx >=
        this.state.LEFT_LIMIT - this.props.BORDER_WIDTH &&
      this.state.leftPosition.x._offset +
        gesture.dx +
        this.props.BORDER_WIDTH +
        1 <
        this.state.rightPosition.x._offset
    );
  };
  isAllowedToMoveBottomSide = gesture => {
    return (
      this.state.bottomPosition.y._offset + gesture.dy <=
        SCREEN_HEIGHT - this.state.BOTTOM_LIMIT &&
      this.state.topPosition.y._offset + this.props.BORDER_WIDTH + 1 <
        this.state.bottomPosition.y._offset + gesture.dy
    );
  };
  isAllowedToMoveRightSide = gesture => {
    return (
      this.state.rightPosition.x._offset + gesture.dx <=
        SCREEN_WIDTH - this.state.RIGHT_LIMIT &&
      this.state.leftPosition.x._offset + this.props.BORDER_WIDTH + 1 <
        this.state.rightPosition.x._offset + gesture.dx
    );
  };
  ...
  initRectanglePanResponder = () => {
    return PanResponder.create({
      ...
      onPanResponderRelease: () => {
        ...
        const width =
          this.state.rightPosition.x._value -
          this.state.leftPosition.x._value -
          this.props.BORDER_WIDTH;
        const height =
          this.state.bottomPosition.y._value -
        ...
        if (
          ...
          Animated.parallel([
            Animated.spring(this.state.leftPosition.x, {
              toValue: this.state.LEFT_LIMIT - this.props.BORDER_WIDTH,
            }),
            Animated.spring(this.state.rightPosition.x, {
              toValue: this.state.LEFT_LIMIT + width,
            }),
          ]).start(() => {
            this.isRectangleMoving = false;
          });
        ...
        if (
          ...
          Animated.parallel([
            Animated.spring(this.state.topPosition.y, {
              toValue: this.state.TOP_LIMIT - this.props.BORDER_WIDTH,
            }),
            Animated.spring(this.state.bottomPosition.y, {
              toValue: this.state.TOP_LIMIT + height,
            }),
          ]).start(() => {
            this.isRectangleMoving = false;
          });
        ...
        if (
          width + this.state.leftPosition.x._value + this.props.BORDER_WIDTH >
          SCREEN_WIDTH - this.state.RIGHT_LIMIT
        ) {
          isOutside = true;
          Animated.parallel([
            Animated.spring(this.state.leftPosition.x, {
              toValue:
                SCREEN_WIDTH -
                this.state.RIGHT_LIMIT -
                width -
                this.props.BORDER_WIDTH,
            }),
            Animated.spring(this.state.rightPosition.x, {
              toValue: SCREEN_WIDTH - this.state.RIGHT_LIMIT,
            }),
          ]).start(() => {
            this.isRectangleMoving = false;
          });
        ...
        if (
          height + this.state.topPosition.y._value + this.props.BORDER_WIDTH >
          SCREEN_HEIGHT - this.state.BOTTOM_LIMIT
        ) {
          isOutside = true;
          Animated.parallel([
            Animated.spring(this.state.topPosition.y, {
              toValue:
                SCREEN_HEIGHT -
                this.state.BOTTOM_LIMIT -
                height -
                this.props.BORDER_WIDTH,
            }),
            Animated.spring(this.state.bottomPosition.y, {
              toValue: SCREEN_HEIGHT - this.state.BOTTOM_LIMIT,
            }),
          ]).start(() => {
            this.isRectangleMoving = false;
          });
  ...
  onRotate = () => {
    ...
    if (this.state.rotation % 180 === 90) {
      ...
      imageWidth =
        SCREEN_WIDTH - this.state.LEFT_LIMIT - this.state.RIGHT_LIMIT;
      imageHeight =
        SCREEN_HEIGHT - this.state.TOP_LIMIT - this.state.BOTTOM_LIMIT;
      rotation = 90;
    }
    const {
      TOP_LIMIT,
    ...
    const startPositionBeforeRotationX =
      this.state.leftPosition.x._value -
      this.state.LEFT_LIMIT +
      this.props.BORDER_WIDTH;
    const startPositionBeforeRotationY =
      this.state.topPosition.y._value -
      this.state.TOP_LIMIT +
      this.props.BORDER_WIDTH;
    const imageWidthBeforeRotation =
      SCREEN_WIDTH - this.state.RIGHT_LIMIT - this.state.LEFT_LIMIT;
    const imageHeightBeforeRotation =
      SCREEN_HEIGHT - this.state.BOTTOM_LIMIT - this.state.TOP_LIMIT;
    const rectangleWidthBeforeRotation =
      this.state.rightPosition.x._value -
      this.state.leftPosition.x._value -
      this.props.BORDER_WIDTH;
    const rectangleHeightBeforeRotation =
      this.state.bottomPosition.y._value -
      this.state.topPosition.y._value -
      this.props.BORDER_WIDTH;
    ...
    const rectangleWidthAfterRotation =
      (imageWidthAfterRotation * rectangleHeightBeforeRotation) /
      imageHeightBeforeRotation;
    const rectangleHeightAfterRotation =
      (imageHeightAfterRotation * rectangleWidthBeforeRotation) /
      imageWidthBeforeRotation;
    const startPositionAfterRotationX =
      (startPositionBeforeRotationY * imageWidthAfterRotation) /
      imageHeightBeforeRotation;
    const startPositionAfterRotationY =
      ((imageWidthBeforeRotation -
        startPositionBeforeRotationX -
